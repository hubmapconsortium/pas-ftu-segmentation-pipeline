...
if __name__ == "__main__":
    ...
    target = item[
        2
    ]  #  torch.Tensor of size=20 (=nb_classes), contains 3 values: -1 (absence of class), 1 (presence of class), 0 (hard example)
    ...
    for class_id, has_class in enumerate(target):
        if has_class == 1:
            print(
                "image {} has object of class {}".format(
    ...
    tf = transforms.Compose(
        [
            ...
            transforms.CenterCrop(
                224
            ),  # extract a square of size 224 at the center of the image
    ...
    def save_activation(self, input, output):
        ...
        for i in range(10):  # output.data.size(1)):
            path_img_output = os.path.join(
                dir_activations,
                "layer{}_{}_channel{}.png".format(layer_id, self.__class__.__name__, i),
            )
        ...
        if "features" in key and "weight" in key:
            for filter_id in range(3):
                for channel_id in range(3):
                    path_param = os.path.join(
                        dir_parameters,
                        "{}_filter{}_channel{}.png".format(key, filter_id, channel_id),
                    )
                    print("save " + path_param)
                    torchvision.utils.save_image(
