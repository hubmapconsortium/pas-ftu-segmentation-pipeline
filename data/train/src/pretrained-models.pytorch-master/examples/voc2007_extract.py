    ...
    for class_id in range(len(classes)):
        ...
        if ignore_hard_examples:
            ...
            train_features = torch.masked_select(
                features[train_split], train_masks.expand_as(features[train_split])
            ).view(-1, features[train_split].size(1))
            train_targets = torch.masked_select(
                targets[train_split], train_masks.expand_as(targets[train_split])
            ).view(-1, targets[train_split].size(1))
            test_masks = (targets[test_split][:, class_id] != 0).view(-1, 1)
            test_features = torch.masked_select(
                features[test_split], test_masks.expand_as(features[test_split])
            ).view(-1, features[test_split].size(1))
            test_targets = torch.masked_select(
                targets[test_split], test_masks.expand_as(targets[test_split])
            ).view(-1, targets[test_split].size(1))
        else:
        ...
        train_y = (
            train_targets[:, class_id] != -1
        ).numpy()  # uses hard examples if not ignored
        ...
        print(
            'class "{}" ({}/{}):'.format(
                classes[class_id], test_y.sum(), test_y.shape[0]
            )
        )
        print(
            "  - {:8}: acc {:.2f}, AP {:.2f}".format(train_split, train_acc, train_AP)
        )
...
def main():
    ...
    model.last_linear = (
        pretrainedmodels.utils.Identity()
    ...
    if args.train_split == "train" and args.test_split == "val":
        ...
        raise ValueError(
            "Trying to train on {} and eval on {}".format(
