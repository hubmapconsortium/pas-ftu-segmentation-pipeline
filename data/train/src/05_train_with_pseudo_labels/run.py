...
def run(seed, data_df, pseudo_df, trn_idxs_list, val_idxs_list):
    ...
    for fold, (trn_idxs, val_idxs) in enumerate(zip(trn_idxs_list, val_idxs_list)):
        ...
        for epoch in range(1, config["num_epochs"] + 1):
            ...
            trn_df_0 = trn_df[trn_df["is_masked"] == False].sample(
                n_sample, replace=True
            )
            trn_df_1 = trn_df[trn_df["is_masked"] == True].sample(
                n_sample, replace=True
            )
            ...
            for bin_size in trn_df_1["binned"].unique():
                trn_df_list.append(
                    trn_df_1[trn_df_1["binned"] == bin_size].sample(n_bin, replace=True)
                )
            trn_df_1 = pd.concat(trn_df_list, axis=0)
            trn_df_balanced = pd.concat([trn_df_1, trn_df_0], axis=0).reset_index(
                drop=True
            )
            ...
            for i, data in enumerate(tk0):
                ...
                with torch.cuda.amp.autocast():
                    ...
                    if config["clfhead"]:
                        ...
                        if config["deepsupervision"]:
                            logits, logits_deeps, logits_clf = model(
                                data["img"].to(device, torch.float32, non_blocking=True)
                            )
                        else:
                            logits, logits_clf = model(
                                data["img"].to(device, torch.float32, non_blocking=True)
                            )
                    else:
                        if config["deepsupervision"]:
                            logits, logits_deeps = model(
                                data["img"].to(device, torch.float32, non_blocking=True)
                            )
                        else:
                            logits = model(
                    ...
                    dice_numer, dice_denom = dice_sum_2(
                ...
                counter += 1
                tk0.set_postfix(
                    loss=(running_loss_trn / (counter * train_loader.batch_size))
                )
            ...
            for i, data in enumerate(tk1):
                with torch.no_grad():
                    ...
                    loss_val += (
                        lovasz_hinge(
                            logits.view(-1, h, w), y_true.view(-1, h, w)
                        ).item()
                        * batch
                    )
                    if config["deepsupervision"]:
                        for logits_deep in logits_deeps:
                            loss_val += (
                                0.1
                                * criterion_lovasz_hinge_non_empty(
                                    criterion, logits_deep, y_true
                                ).item()
                                * batch
                            )
                    if config["clfhead"]:
                        loss_val += (
                            criterion_clf(logits_clf.squeeze(-1), y_clf).item() * batch
                        )
                # release GPU memory cache
            ...
            # logging
            log_df.loc[log_counter, log_cols] = np.array(
                [
                    fold,
                    epoch,
            ...
            # monitering
            print(
                "epoch {:.0f} loss_trn = {:.5f}, loss_val = {:.5f}, trn_score = {:.4f}, val_score = {:.4f}".format(
                    epoch, epoch_loss_trn, loss_val, trn_score, val_score
                )
            )
            if epoch % 10 == 0:
                print(
            ...
            if config["early_stopping"]:
                if loss_val < loss_val_best:  # val_score > val_score_best:
                    ...
                    torch.save(
                        model.state_dict(),
                        output_path + f"model_seed{seed}_fold{fold}_bestloss.pth",
                    )  # save
                ...
                if counter_ES > config["patience"]:
                    print(
                        "early stopping, epoch_best {:.0f}, loss_val_best {:.5f}, val_score_best {:.5f}".format(
                            epoch_best, loss_val_best, val_score_best
                        )
                    )
                    break
            else:
            ...
            if val_score > val_score_best2:
                ...
                torch.save(
                    model.state_dict(),
                    output_path + f"model_seed{seed}_fold{fold}_bestscore.pth",
                )  # save
            ...
            if config["lr_scheduler_name"] == "CosineAnnealingLR":
                ...
                if (
                    ...
                    torch.save(
                        model.state_dict(),
                        output_path + f"model_seed{seed}_fold{fold}_epoch{epoch}.pth",
                    )  # save
        ...
        if config["early_stopping"] & (counter_ES <= config["patience"]):
            print(
                "epoch_best {:d}, val_loss_best {:.5f}, val_score_best {:.5f}".format(
                    epoch_best, loss_val_best, val_score_best
                )
            )
