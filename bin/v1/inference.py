...
class UNET_RESNET34(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.config["clf_threshold"] is not None:
            if (
                torch.sigmoid(logits_clf) > self.config["clf_threshold"]
            ).sum().item() == 0:
...
def build_model(resolution, deepsupervision, clfhead, load_weights, config):
    ...
    if model_name == "resnet34":
        model = UNET_RESNET34(
            resolution, deepsupervision, clfhead, config, load_weights
        )
    elif model_name == "seresnext50":
        model = UNET_SERESNEXT50(
            resolution, deepsupervision, clfhead, config, load_weights
        )
    elif model_name == "seresnext101":
        model = UNET_SERESNEXT101(
            resolution, deepsupervision, clfhead, config, load_weights
        )
    return model
...
def get_model_lists(config):
    ...
    for seed in config["split_seed_list"]:
        ...
        for path in LOAD_LOCAL_WEIGHT_PATH_LIST[seed]:
            ...
            model = build_model(
                resolution=(None, None),  # config['resolution'],
...
def get_pred_mask(path, model_list, config):
    ...
    for data in tqdm(dl):
        ...
        pred_mask_float = (
            pred_mask_float / min(config["tta"], 4) / len(model_list[seed])
        ...
        for j in range(bs):
            ...
            pred_mask[i_data + j, 0 : py1 - py0, 0 : px1 - px0] = pred_mask_int[
...
def get_rle(y_preds, h, w, config):
    rle = mask2rle(
        y_preds, shape=(h, w), small_mask_threshold=config["small_mask_threshold"]
    )
    return rle
...
def mask2json(mask):
    ...
    polygons = [
        [[int(point[1]), int(point[0])] for point in object] for object in contours
    ]
    # save as json
    geojson_dict_template = {
        ...
        "properties": {
            "classification": {"name": "glomerulus", "colorRGB": -3140401},
            "isLocked": True,
            "measurements": [],
        },
    }
...
def main(data_directory: Path, tissue_type: str):
    ...
    if tissue_type not in supported_tissue_types:
        raise ValueError(
            f"Type {tissue_type} not supported, only: {', '.join(supported_tissue_types)}"
        )
